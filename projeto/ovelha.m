...
prima_1=false; prima_2=false; opcao=0; linear_1=0; linear_2=0; 
carga_definida=false; fonte_definida=false;
...
while prima_1 == false || prima_2 == false
    ...
    while opcao < 1 || opcao > 2 || ~isscalar(opcao)
        ...
        if opcao ==1 && carga_definida==false
            prima_1=true; carga_definida=true;
            clc;
            ...
            if linear_1 == 2 
                ...
                clc;
                c=input('Escreva a reta da carga não linear: ');
                carga=@(x) c;
        ...
        if opcao ==2 && fonte_definida==false
            prima_2=true; fonte_definida=true;
            clc;
            fprintf('Prima 1: Linear ');
            fprintf('\nPrima 2 : Não Linear ');
            ...
            if linear_2 == 2 
                clc;
                f=input('Escreva a reta da fonte não linear: ');
                fonte=@(x) f;
                clc;
                %f=strcat('@(x)', ['(' f ')']);
                %f=eval(f);
...
plot(x, carga(x), 'LineWidth', 2); hold on;
plot(x, fonte(x), 'LineWidth', 2);
ylim([0 14]); 
